...
| V-16 | Assign / withdraw a product | `assign_vendor_product_to_retailer()`, `unassign_vendor_product_from_retailer()` ✅ **adopted and reused verbatim — NO migration, NO new RPC** | **C** | **The assignment-writes milestone found no backend gap.** The whole web path — `/products/[productId]` → `assignProductAction`/`unassignProductAction` → `lib/products/vendor-products.ts` → RPC — already contains **no direct table write, no service-role client, no caller-supplied tenant id and no TypeScript-only validation rule**; both writes accept **no text input at all**, so unlike V-13/V-14 they have no normalization path to a raw constraint error. **Permission is `PRODUCT_RETAILER_ASSIGN`, genuinely distinct from `PRODUCTS_MANAGE`** — proved by removing each seeded mapping in turn in pgTAP: `PRODUCTS_MANAGE` alone does **not** grant assignment; `PRODUCT_RETAILER_ASSIGN` alone **is** sufficient for both writes. **Create and reactivate are ONE call** (`assign` inserts, or flips an existing `INACTIVE` row back); **withdraw is separate and deliberately weaker** — it requires **no** status to be `ACTIVE`, so a product can still be withdrawn from a suspended relationship, a suspended Retailer organization, or while the product itself is `INACTIVE`. Assigning requires product **and** relationship **and** Retailer org all `ACTIVE`; reactivation goes through the same gate. **Withdrawal never deletes** — one row per (product, Retailer) **for all time** under an unpartial unique index, and there is no `DELETE` in either function. ⚠️ **`assigned_at` is OVERWRITTEN with `now()` on reactivation** (withdrawal preserves it), so it is the *current* assignment's start, **not** the pairing's first — do not label it "first assigned"; the pairing's full history lives in the audit log. **Address-space question settled:** addressed by `retailer_organization_id`, and **not** re-addressed by `relationship_id` — the table stores no relationship id and the read contract makes `relationship_id` nullable, which would leave exactly the historical rows un-withdrawable. V-12b and V-05 each return **both** ids on the same row, so no second lookup is needed. Same-status requests are **silent no-ops that write no row version and no audit row**, which is what stops a double-tap producing two audit entries. All four concurrency races serialize with zero errors and zero duplicate rows. Audit (`PRODUCT_ASSIGNED_TO_RETAILER` / `PRODUCT_UNASSIGNED_FROM_RETAILER`, five display-only metadata keys) is **transactional** — an audit failure rolls the mutation back, proved with a failing-audit trigger. `void` return: treat a no-op as success and refresh through V-12a + V-12b. Full audit: `docs/mobile-vendor-product-assignment-writes-audit.md`. |
