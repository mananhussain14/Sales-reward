...
| Change an existing Sales Staff member's shops | **Not built (backend only)** | 🟢 Ready | 🟢 Now | ✅ **shipped** `set_retailer_staff_shop_assignments(p_membership_id, p_shop_ids)` | No | **No UI exists on web or mobile — this milestone is backend only.** Send the canonical `membership_id` from `list_retailer_staff_members()` and the **complete** desired shop set; the diff is computed in SQL. Duplicates are canonicalized; an empty/null list is `23514` (there is no "stand down" operation). Returns `(shops_added, shops_removed, shops_unchanged)`. ⚠️ Replacement is scoped to **ACTIVE** shops: a live assignment to a suspended/deactivated shop is invisible in the roster, is **preserved**, and is in **none** of the three counts — so never present them as the member's total shops. Full contract: `docs/retailer-staff-shop-assignment-management-audit.md`. | **2** |
