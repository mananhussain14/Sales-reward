...
| Explain a refused sign-in / empty app ("why can't I get in?") | **Not built** | 🟢 Ready | 🔴 Not implemented | ✅ **shipped** `get_my_lifecycle_access_state()` | No | **Backend only — neither client implements it yet.** Zero arguments, self-only: the subject is `auth.uid()` and there is nothing to substitute. Returns one word from a closed set — `ACTIVE`, `PROFILE_INACTIVE`, `MEMBERSHIP_INACTIVE`, `ORGANIZATION_INACTIVE`, `NO_SUPPORTED_ACCESS`, `AMBIGUOUS` — and no id, name, email, role, timestamp or database message. Precedence is profile → organization → membership. ⚠️ **It is a diagnostic, never an authorization gate:** call it on the **refusal** path to choose a sentence, never instead of calling the operation. Treat an unrecognised value as unknown and fall back to generic copy. Separate from `get_my_portal_context()`, which is unchanged — that reports `NONE` for both an inactive profile and a Vendor-only user, and only this can tell them apart. Contract: **AUTH-06** and `docs/retailer-staff-membership-lifecycle-audit.md`. | **1** |
...
| Change an existing Sales Staff member's shops | Shipped | 🟢 Ready | 🔴 Not implemented for Manage Shops | ✅ **shipped** `set_retailer_staff_shop_assignments(p_membership_id, p_shop_ids)` | No | **Backend shipped; web shipped (`/retailer/staff` → Manage shops); Flutter not yet implemented for Manage Shops.** Send the canonical `membership_id` from `list_retailer_staff_members()` and the **complete** desired shop set; the diff is computed in SQL. Duplicates are canonicalized; an empty/null list is `23514` (there is no "stand down" operation). Returns `(shops_added, shops_removed, shops_unchanged)`. ⚠️ Replacement is scoped to **ACTIVE** shops: a live assignment to a suspended/deactivated shop is invisible in the roster, is **preserved**, and is in **none** of the three counts — so never present them as the member's total shops; re-read the roster for display. Contracts: `docs/retailer-staff-shop-assignment-management-audit.md` (database) and `docs/retailer-manage-staff-shops-web.md` (web client). | **2** |
| Deactivate / reactivate a staff member | **Not built** | 🟢 Ready | 🔴 Not implemented | ✅ **shipped** `set_retailer_staff_membership_status(p_membership_id, p_status)` | No | **Backend only — no web UI and no Flutter; neither client implements it yet.** Send the canonical `membership_id` from `list_retailer_staff_members()` and exactly `ACTIVE` or `DEACTIVATED` (case-sensitive; anything else is `23514`). Eligible targets are the **exact** role sets `{RETAILER_MANAGER}` or `{SALES_STAFF}` — every Owner, multi-role and role-less target is `42501`, and so is the caller's own membership. Returns `(membership_id, membership_status, role_code, status_changed)`; use `status_changed` to tell "saved" from "already in that state". Idempotent: a repeat writes nothing, not even an audit row. ⚠️ **`auth.users` is untouched, so a deactivated person can still sign in** — they simply lose context, from their very next request on the **same session**. Nothing is deleted, so reactivation restores roles and Shop assignments automatically. All seven target refusals share one SQLSTATE **and one message**; never surface the raw code or the database message. Contract: `docs/retailer-staff-membership-lifecycle-audit.md`. | **2** |
...
| Deactivate / reactivate a Retailer | **Not built** | 🟢 Ready | 🔴 Not implemented | ✅ **shipped** `set_vendor_retailer_status(p_relationship_id, p_status)` — migration `20260811090000` | No | **Backend only — neither client implements it yet.** New permission `RETAILERS_MANAGE`, mapped to `VENDOR_SUPER_ADMIN` alone. Writes **both** `vendor_retailers.status` **and** the Retailer's `organizations.status` in one transaction — the relationship row alone blocks nobody, because no Retailer-side resolver consults it, while `organizations.status` refuses every Retailer Owner, Manager and Sales Staff request **on an already-issued session's next call**. ⚠️ **Stored vocabulary is `ACTIVE` / `SUSPENDED`; the UI word is Deactivate / Inactive / Reactivate** — do not send `INACTIVE`, and `DEACTIVATED` is neither requestable nor clearable. Returns `(relationship_id, retailer_status, relationship_status, status_changed)`; a repeat is a no-op with `status_changed = false` and no audit row. Nothing is cascaded into memberships and `auth.users` is untouched, so reactivation restores roles, Shops, assignments and still-valid invitations automatically. Refusals: `42501` (one literal for every authorization/target case), `23514` (bad status), `55000` (one literal for inconsistent pair, `DEACTIVATED` row, row-count drift **and** the multi-Vendor guard). ⚠️ **The multi-Vendor guard is a stopgap** — tenant blocking must be redesigned before a Retailer may have two active Vendors. Contract: **V-20** and `docs/vendor-retailer-lifecycle-audit.md`. | **3** |
