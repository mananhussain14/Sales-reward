...
-- earnings reads and calls no evaluator at all. Migration 71 (20260828210000) adds Azure receipt-extraction readiness and likewise touches no campaign evaluator. Migration 72
-- (20260829090000) corrects retry eligibility inside one receipt-extraction read and
-- touches no campaign evaluator either. The rule this assertion owns is that nothing
-- beyond them exists.
select is((select coalesce(string_agg(version, ',' order by version), 'NONE')
           from supabase_migrations.schema_migrations
           where version > '20260826090000'),
  '20260827090000,20260828090000,20260828210000,20260829090000',
  'A2. the only migrations after 20260826090000 are approved Migrations 69 through 72');
